...
class LowSpdShaft(object):
    ...
    def computeMass(self, RotorDiam, RotorMass, RotorTorque):
        ...
        mmtArm    = self.lenShaft / 5                                                                 
        bendLoad  = 1.25*9.81*RotorMass                                                           
        bendMom   = bendLoad * mmtArm                                                                 
        self.hFact     = 0.1                                                                                    
        hollow    = 1/(1-(self.hFact)**4)                                                                   
        self.outDiam   = ((32/pi)*hollow*3.25*((RotorTorque*3/371000000.)**2+(bendMom/71070000)**2)**(0.5))**(1./3.) 
...
class csmNacelle(object):
    ...
    def computeMass(self, RotorDiam,    MachineRating, RotorMass, RotorSpeed, \
        ...
        print [RotorDiam,    MachineRating, RotorMass, RotorSpeed, \
                      MaximumThrust, RotorTorque, iDesign,   offshore, \
                      crane, AdvancedBedplate]
        ...
        self.mainframeMass = mfmCoeff[iDesign] * (RotorDiam ** mfmExp[iDesign] )
    ...
    def computeCost(self, RotorDiam,    MachineRating, RotorMass, RotorSpeed, \
        ...
        MainFrameCost2002 = mfmCoeff[iDesign] * RotorDiam ** mfmExp[iDesign]
